...
def hybrid_extract(text: str, context: str = None, k=15):
    """하이브리드 추출"""
    # 1.5단계: 기계적 context 판단 (LLM 주관성 배제)
    if context is None:
    ...
    for r in vector_results:
        candidates.append(f"{r['label_id']}: {r['text'][:80]}")
    ...
    # JSON 예시를 변수로 분리 (f-string 충돌 방지)
    json_example = '{"selected_labels": [{"label_id": "XX-XX", "confidence": 0.95}]}'
...
[분석 지침]
1. Negation 체크: "하지만", "실제로는", "안 함" 등 부정 표현이 있으면 해당 라벨 confidence 낮춤
2. 최종 라벨 선택: 후보 중 실제로 적용되는 라벨만 선택 (context 정보 참고)
3. Confidence: 명확한 언급 0.9+, 강한 추론 0.7-0.89, 합리적 추론 0.5-0.69
...
"""
    ...
    try:
        ...
        print(f"  LLM 분석 완료")
        return result.get('selected_labels', []), context
    except Exception as e:
        print(f"  LLM 오류: {e}")
        # Fallback: Vector 결과 반환
...
if __name__ == "__main__":
    # 결과 파일 열기 (인코딩 문제 회피)
    with open(PROJECT_ROOT / "data" / "vectors" / "hybrid_results_v2.txt", 'w', encoding='utf-8') as result_file:
    ...
    print("결과 저장 완료: data/vectors/hybrid_results_v2.txt")
